...
| Database | Date Filter | Language Filter | Search Method |
|----------|------------|----------------|---------------|
| PubMed | 2000/01/01 to present ([PDAT]) | English [la] | E-utilities API, Cochrane HSS Sensitivity-Maximizing (2008 revision) |
| Europe PMC | PUB_YEAR:[2000 TO 2026] | LANG:"eng" | REST API (GET), TITLE_ABS + AFF fields |
| OpenAlex | from_publication_date:2000-01-01 | language:en | REST API, 2-pass (affiliation + text), inverted index abstract reconstruction |
| Scopus | PUBYEAR > 1999 | LANGUAGE(english) | Search API (COMPLETE view), TITLE-ABS-KEY + AFFILCOUNTRY, year-based chunking for >5k results |
...
2. **Deduplication**: DOI matching → PMID matching → fuzzy title matching (>0.93 threshold, same year)
3. **Filtering**:
